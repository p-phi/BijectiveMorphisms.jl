@testset "BijectiveMorphism basics" begin
...
@testset "BijectiveMorphism multi-domain" begin
    f = BijectiveMorphism(x -> Char(x), x -> Int(x), Int, Char)
...
@testset "Non invertible bijection must fail" begin
    f = BijectiveMorphism(x -> Char(x), x -> Int(x + 1), Int, Char)
